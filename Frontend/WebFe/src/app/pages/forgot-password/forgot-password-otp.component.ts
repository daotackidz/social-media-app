...
export class ForgotPasswordOtpComponent implements OnInit, OnDestroy {
  ...
  submit(): void {
    ...
    this.authService.verifyForgotPasswordOtp(this.email, code).subscribe({
      next: () => {
        this.isVerifying.set(false);
        this.router.navigate(['/reset-password'], {
          state: { email: this.email, otpCode: code }
        });
      },
      error: (err) => {
        this.isVerifying.set(false);
  ...
  resend(): void {
    ...
    this.authService.forgotPassword(this.email).subscribe({
      next: (response) => {
        ...
        this.digits.set(Array(OTP_LENGTH).fill(''));
        this.expirySeconds.set(response.data?.otpExpiresInSeconds || OTP_VALIDITY_SECONDS);
